...
| 模块 | 文件 | 内容 |
|------|------|------|
| 01 | `tests/01_baseline.py` | 基础功能 — 页面访问/登录/退出/搜索 (34项) |
| 02 | `tests/02_sql_injection.py` | SQL 注入 — 9个注入点 + 编码绕过 (18项) |
| 03 | `tests/03_xss.py` | XSS — 评论/留言存储型 (22项) |
| 04 | `tests/04_command_injection.py` | 命令注入 — avatar_url 10种payload (12项) |
| 05 | `tests/05_path_traversal.py` | 路径遍历 — 文件上传恶意文件名 (11项) |
| 06 | `tests/06_auth_bypass.py` | 认证绕过 — 万能密码/伪造session/暴力破解 (10项) |
| 07 | `tests/07_crawler.py` | 全站爬虫 — 提取链接/表单/站点地图JSON |
| 08 | `tests/08_api_fuzzing.py` | API模糊 — 参数变异/关键字/方法探测 (31项) |
...
│   ├── lib/
│   │   ├── __init__.py
│   │   └── test_utils.py       # 公共库（HTTP客户端、断言、报告）
│   ├── 01_baseline.py
│   ├── 02_sql_injection.py
│   ├── 03_xss.py
